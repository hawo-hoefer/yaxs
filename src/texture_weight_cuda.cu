...
__device__ float quat_mag(Quaternion q) { return sqrtf(q.w * q.w + q.x * q.x + q.y * q.y + q.z * q.z); }

#define quat_fmt "%6.3f %6.3f %6.3f %6.3f"
...
struct FFIData {
  ...
  Quaternion *sample_to_bingham; // [n_permutations]
 ...
 */
__device__ size_t permutation_index(size_t weight_index, size_t stride, Permutations permutations) {
  size_t hkl_acc = 0;
  size_t permutation_idx = 0;

  for (size_t i = 0; i < permutations.n_permutations; ++i) {
    hkl_acc += permutations.hkl_sizes[i] * stride;
    if (hkl_acc > weight_index)
      break;
...
__device__ size_t permutation_start_hkls(size_t perm_idx, Permutations permutations) {
  size_t acc = 0;

  assert(perm_idx < permutations.n_permutations);
  for (size_t i = 0; i < perm_idx; ++i) {
    acc += permutations.hkl_sizes[i];
  }
  return acc;
}
...
__global__ void compute_hkl_weight(Quaternion *beam_to_domain, Vec3 *h, float *w, Permutations permutations,
  ...
  size_t hkl_perm_start = permutation_start_hkls(permutation_idx, permutations);
  size_t hkls_in_permutation = permutations.hkl_sizes[permutation_idx];

  size_t alignment_idx = (weight_index - hkl_perm_start * stride_in_alignments) / hkls_in_permutation;
...
__global__ void precompute_beam_to_sample(float *chis, float *phis, Quaternion *beam_to_sample, size_t n_chis,
                                          size_t n_phis) {
  size_t tid = blockDim.x * blockIdx.x + threadIdx.x;
  ...
  beam_to_sample[tid] = beam_to_sample_tf(chi, phi);
}
 ...
 */
__global__ void precompute_beam_to_bingham(Quaternion *beam_to_sample, Quaternion *sample_to_bingham,
  ...
  Quaternion beam2s = beam_to_sample[alignment_idx];
  Quaternion s2b = sample_to_bingham[perm_idx];
  beam_to_bingham[tid] = q_hamilton(beam2s, s2b);
}
 ...
 */
__global__ void compute_beam_to_domain(Quaternion *beam_to_bingham, Quaternion *base_orientation_samples,
                                       Quaternion *beam_to_domain, size_t stride, size_t ori_samples_per_alignment,
                                       size_t n_permutations) {
  ...
  size_t bingham_to_beam_idx = perm_idx * stride + alignment_idx;
  size_t base_orientation_idx = ori_sample_idx;

  Quaternion beam2bing = beam_to_bingham[bingham_to_beam_idx];
  Quaternion bing2domain = base_orientation_samples[base_orientation_idx];
  Quaternion beam2domain = q_unit_recip(q_hamilton(beam2bing, bing2domain));
  beam_to_domain[tid] = beam2domain;
}
...
bool weighted_i_hkls_single_structure(FFIData ffidata, Permutations permutations, float *i_hkls, float kappa,
  ...
  snprintf(tmp_str_buf, sizeof(tmp_str_buf),
           ...
           (float)sample_to_bingham_space / 1e6,
  ...
  Quaternion *beam_to_sample;
  Quaternion *sample_to_bingham; // n_permutations long array of bingham orientations for each permutation
  ...
  cu_lerr(cudaMalloc(&beam_to_sample,           alignment_tf_space),       "allocating alignment transformations");
  cu_lerr(cudaMalloc(&beam_to_bingham,          beam_to_bingham_space),    "allocating beam_to_bingham orientations");
  cu_lerr(cudaMalloc(&sample_to_bingham,        sample_to_bingham_space),  "allocating bingham distribution orientations");
  ...
  launch_kernel_sensibly_no_shmem(precompute_beam_to_sample, "precomputing alignment transformations", stride, chis,
                                  phis, beam_to_sample, ffidata.n_chis, ffidata.n_phis);
  ...
  launch_kernel_sensibly_no_shmem(precompute_beam_to_bingham, "precomputing beam to bingham transformations",
                                  permutations.n_permutations * stride, beam_to_sample, sample_to_bingham,
  ...
  launch_kernel_sensibly_no_shmem(compute_beam_to_domain, "computing tranformed orientations",
  ...
  cudaFree(beam_to_sample);
  cudaFree(sample_to_bingham);
